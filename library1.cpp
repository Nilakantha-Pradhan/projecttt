#include <iostream> //calling all the required library's
#include <stdlib.h>
#include <conio.h>
#include <string.h>
using namespace std;
//Centurion University Central Library :-)//
struct student // created a class student structure
{
	int bookID;
	char title[30];
...
void Add_Book() //ENTER THE BOOK RECORD
{
	if (arr < books)
	{
		...
		cout << "isbn no. of the book= ";
		cin >> studentobj[arr].bookID;
		cout << "Book name  ";
		cin >> studentobj[arr].title;
		cout << "cost of book= ";
		cin >> studentobj[arr].costofbook;
		cout << "for how many days yo want the book= ";
		cin >> studentobj[arr].duration;
		arr++;
	}
	else
	{
...
int main() //MAIN BODY START
{
	...
	cout << "\tCENTURION UNIVERSITY CENTRAL LIBRARY\n\n";
	do
	{
		cout << "Username: ";
		cin >> username;
		...
		if (username == "123" && password == "cutm123")
		{
			do
			{
